...
> **물건이 4건 이상이면 비교표는 물건을 열이 아닌 행으로** 배치(열은 지표). 세대·연식은 apt-value 값,
> 권리 상태는 pill(인수0/인수위험/미수집), 입지는 6축 등급 배지로 요약한다.
> 입지분석을 안 했으면 그 사실을 부록에 **명시**(레이더 억지로 채우지 말 것).
> **즉시 열람용 공유가 필요하면 GitHub Pages 배포 대신 `Artifact`로 발행**(기본 비공개)해 링크만 줘도 된다.
> 단, 개인정보·특정 개인 대상 내용이 없는 공개데이터 분석일 때만.
